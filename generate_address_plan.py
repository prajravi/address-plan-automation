...
def load_config():
    """Load VLAN, site type, and shared connectivity configuration."""
    ...
    connectivity_cfg = load_yaml(os.path.join(SCRIPT_DIR, "config", "connectivity.yaml"))
    return vlans_cfg, site_types_cfg, connectivity_cfg
...
class AddressPlan:
    def __init__(self, input_cfg):
        ...
        self.vlans_cfg, self.site_types_cfg, self.connectivity_cfg = load_config()
        self.subnet_sizes = self.vlans_cfg["subnet_allocation"][self.site_type]
        try:
            self.topology = self.connectivity_cfg[self.site_type]
        except KeyError as exc:
            raise ValueError(f"No connectivity is defined for site type: {self.site_type}") from exc
    ...
    def _row(self, subnet="", ipv4="", ipv6="", dns="", a_rec="", aaaa=""):
        self.rows.append([subnet, ipv4, ipv6, dns, a_rec, aaaa])

    def _blank(self):
        self._row()

    def _topology_device_name(self, role, dev_id, context=None):
        """Build a site-prefixed device name from a shared connectivity role."""
        if role == "floor_sw":
            if not context:
                raise ValueError("Floor-switch connectivity requires floor and stack context")
    ...
    def _connection_enabled(self, connection):
        """Return whether an optional shared connectivity entry applies to this site."""
        condition = connection.get("condition")
        if not condition:
            return True
        service = condition[:-8] if condition.endswith("_enabled") else condition
        return self.cfg.get("services", {}).get(service, False)
    ...
    def _generate_crosslinks(self):
        """Generate addressable links from the shared connectivity definition."""
        site = self.site
        ...
        wan_transit_seen = False
        for link in self.topology["connections"]:
            if not self._connection_enabled(link):
                continue

            is_wan_transit = link["to_role"] == "ISP"
            if wan_transit_seen and not is_wan_transit:
                self._blank()
                wan_transit_seen = False

            contexts = [{}]
            if "floor_sw" in (link["from_role"], link["to_role"]):
                contexts = [
            ...
            for context in contexts:
                device_a = self._topology_device_name(
                    link["from_role"], link["from_id"], context
                )
                if is_wan_transit:
                    wan_offset = self.crosslink_offset
                    wan_subnet = self._next_crosslink()
                    wan_v6 = self._infra_v6_wan_transit(wan_offset)
                    self._row(
                        f"{device_a} <-> WAN", str(wan_subnet),
                        str(wan_v6) if wan_v6 else "",
                    )
                    wan_transit_seen = True
                    continue

                device_b = self._topology_device_name(
                    link["to_role"], link["to_id"], context
                )
                self._crosslink_entry(
                    f"{device_a} <-> {device_b}",
                    device_a, link["from_port"],
                    device_b, link["to_port"],
                )
